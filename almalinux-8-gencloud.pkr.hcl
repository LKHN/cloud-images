...
source "qemu" "almalinux-8-gencloud-x86_64" {
  ...
  vm_name            = "AlmaLinux-8-GenericCloud-8.7-beta-1-${formatdate("YYYYMMDD", timestamp())}.x86_64.qcow2"
...
source "qemu" "almalinux-8-gencloud-uefi-x86_64" {
  ...
  net_device         = "virtio-net"
  qemu_binary        = var.qemu_binary
  vm_name            = "AlmaLinux-8-GenericCloud-UEFI-8.7-beta-1-${formatdate("YYYYMMDD", timestamp())}.x86_64.qcow2"
  boot_wait          = var.boot_wait
...
source "qemu" "almalinux-8-gencloud-aarch64" {
  ...
  net_device         = "virtio-net"
  qemu_binary        = var.qemu_binary
  vm_name            = "AlmaLinux-8-GenericCloud-8.7-beta-1-${formatdate("YYYYMMDD", timestamp())}.aarch64.qcow2"
  boot_wait          = var.boot_wait
...
source "qemu" "almalinux-8-gencloud-ppc64le" {
  ...
  net_device         = "virtio-net"
  qemu_binary        = var.qemu_binary
  vm_name            = "AlmaLinux-8-GenericCloud-8.7-beta-1-${formatdate("YYYYMMDD", timestamp())}.ppc64le.qcow2"
